...
function comp_K_S_D_wage(;dist_ss=dist_ss,params = params,w_t=w_t) # the semicolon is important here, it makes sure that the function can only be called with named arguments
    # you can also do solve_9(50,0,4,.8,...) but you have to remember the order of the arguments, much can go wrong...         
    # unpack parameters
    @unpack B_num,B_lower,B_upper,y_low,y_high,p_y,β,r,α,δ,γ,max_iter_HJB,tol_HJB = params

    # set up grid space
    B_grid = w_t*collect(LinRange(B_lower,B_upper,B_num))
    y_grid = w_t*[y_low;y_high]
    pi_mat = [p_y 1-p_y;1-p_y p_y]
    y_num = size(y_grid)[1]
    B_grid_large = kron(ones(y_num),B_grid) 
    y_grid_large = kron(y_grid,ones(B_num))
