...
# this file tests github autopilot 
function solve_v_wage(;params = params,v_int=v_int) # the semicolon is important here, it makes sure that the function can only be called with named arguments
    # you can also do solve_9(50,0,4,.8,...) but you have to remember the order of the arguments, much can go wrong...         
    # unpack parameters
    @unpack B_num,B_lower,B_upper,y_low,y_high,p_y,β,r,α,δ,A_tfp,γ,max_iter_HJB,tol_HJB = params
    ...
    w_t = ((α/(r+δ))^(1/(1-α)))^α*A_tfp*(1-α) # wage rate    
    B_grid = B_grid*w_t # scale the grid with the wage rate! you don't have to but I like it cause if the wage goes up, the grid goes up!
    y_grid = y_grid*w_t # scale the grid with the wage rate! you don't have to but I like it cause if the wage goes up, the grid goes up!
...
#    end
    ...
    c_sol = (1+r)*kron(ones(y_num),B_grid)-B_next + kron(y_grid,ones(B_num))
    B_grid_large = kron(ones(y_num),B_grid) # this is just for plotting purposes, and to remind us how we orderd the states encoded in the vector
    return (w_t=w_t,c_sol=c_sol,convergence_v=convergence_v,v_vec=v_vec,B_grid_large = B_grid_large,B_num=B_num,y_grid_large = kron(y_grid,ones(B_num)),pi_mat=pi_mat,B_next=B_next)
end