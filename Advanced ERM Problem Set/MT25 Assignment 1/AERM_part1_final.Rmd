...
The true population regression slope is $\beta = 2\rho - 1 = 0.9$, since $E[y_{t+1}|y_t]$ is linear in $y_t$ with slope $2\rho - 1$ when $y \in \{1,2\}$. 

OLS is therefore closer to the truth, because the DGP contains no individual heterogeneity, so $E[y_{i,t-1} u_{i,t}] = 0$ and OLS is consistent. 

The FE bias is systematic: it increases with ρ, because the estimator suffers from Nickell bias, ie. demeaning over a short panel ($T=8$) induces a mechanical negative correlation between $\tilde{y}_{i,t-1}$ and $\tilde{u}_{i,t}$, biasing $\hat{\beta}_{FE}$ substantially downward. 
...
| Counterfactual | Asset Supply |
|----------------|-------------|
| $B(\rho = 0, \, c = c^{.5})$   | $-0.1150$ |
| $B(\rho = .95, \, c = c^{.5})$ | $\phantom{-}0.5655$ |
