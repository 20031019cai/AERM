...
function solve_household(p::ModelParams, ρ::Float64)
    ...
    V = -ones(n_states)
    V_new = similar(V)
    policy_idx = zeros(Int, n_states)  # stores index of optimal b'
    ...
    for iter in 1:p.max_iter
        ...
        V_mat = reshape(V, nb, ny)          # nb × ny
        EV_mat = V_mat * Π'                 # nb × ny (expected V for each b' and current y)
        EV = vec(EV_mat)                    # back to vector, same ordering as V

        # Maximise over b' for each state (b, y)
        for iy in 1:ny
            EV_slice = EV[(iy-1)*nb+1 : iy*nb]  # EV(b', y) for current y
            for ib in 1:nb
                idx = (iy - 1) * nb + ib
                best_val = -Inf
                best_ib = 1
